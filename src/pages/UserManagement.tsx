...
export default function UserManagement() {
        ...
        if (createError) {
          ...
          if (authError) {
            if (authError.message.includes('security purposes') || authError.message.includes('rate limit')) {
              throw new Error('Rate limit reached. Please run the SQL script in supabase/schema.sql in your Supabase SQL Editor to enable secure user creation without rate limits.');
            }
            throw authError;
          }
  ...
  return (
    <div className="space-y-6">
      ...
      {userToDelete && (
        <div className="relative z-50" aria-labelledby="modal-title" role="dialog" aria-modal="true">
          <div className="fixed inset-0 bg-gray-500 opacity-75 transition-opacity" onClick={() => setUserToDelete(null)}></div>
                    ...
                    <div className="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                      ...
                      <div className="mt-2">
                        <p className="text-sm text-gray-500">
                          Are you sure you want to delete the user <strong>{userToDelete.name}</strong>? This action cannot be undone and will permanently remove their access to the system.
                        </p>
                      </div>
                    </div>
                  </div>
